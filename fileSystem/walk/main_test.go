...
func TestRunDelExtension(t *testing.T) {
	testCases := []struct {
		...
		{name: "DeleteExtensionMatch",
			cfg:         config{ext: ".log", del: true},
			extNoDelete: "", nDelete: 0, nNoDelete: 10,
			expected: ""},
...
func TestRunArchive(t *testing.T) {
	testCases := []struct {
		...
		{name: "ArchiveExtensionsNoMatch",
			cfg:          config{ext: ".log"},
			extNoArchive: ".gz", nArchive: 0, nNoArchive: 10},
		{name: "ArchiveExtensionsMatch",
			cfg:          config{ext: ".log"},
			extNoArchive: "", nArchive: 10, nNoArchive: 0},
		{name: "ArchiveExtensionsMixed",
			cfg:          config{ext: ".log"},
...
func TestRestore(t *testing.T) {
	testCases := []struct {
		...
		{name: "RestoreExtensionMatch", extArchive: ".go.gz", nArchive: 5, nRestored: 5},
		{name: "RestoreExtensionNoMatch", extArchive: ".go", nArchive: 5, nRestored: 0},
	}

	for _, tc := range testCases {
		var buffer bytes.Buffer

		tmpArchive, archiveCleanup := createTempDir(t, map[string]int{
			tc.extArchive: tc.nArchive,
		})
		defer archiveCleanup()

		tmpRoot, rootCleanup := createTempDir(t, nil)
		defer rootCleanup()
		...
		if err := run(tmpRoot, &buffer, c); err != nil {
			t.Fatal(err)
		}

		pattern := filepath.Join(tmpRoot, "*.go")
		expFiles, err := filepath.Glob(pattern)
		if err != nil {
			t.Fatal(err)
		}
		...
		filesRestored, err := os.ReadDir(tmpRoot)
		if err != nil {
			t.Fatal(err)
		}

		if len(filesRestored) != tc.nRestored {
			t.Errorf("Expected %d files archived, got %d instead\n",
				tc.nRestored, len(filesRestored))
		}
