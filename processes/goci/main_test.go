...
func TestRun(t *testing.T) {
	...
	var testCases = []struct {
		...
		{name: "failBranch", proj: "./testdata/tool", branch: "",
			out:      "",
			expErr:   ErrValidation,
			setupGit: false,
			mockCmd:  nil},
	}
...
func setupGit(t *testing.T, proj string) func() {
	...
	var gitCmdList = []struct {
		...
		{[]string{"config", "credential.helper",
			"'!f() { echo \"username=$GIT_USERNAME\"; echo \"password=$GIT_PASSWORD\"; };f'"}, projPath, nil},
...
func TestHelperProcess(t *testing.T) {
	...
	if os.Args[2] == "golangci-lint" {
		os.Exit(0)
	}

	if os.Args[2] == "gocyclo" {
		os.Exit(0)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			go func() {
				...
				_ = syscall.Kill(syscall.Getpid(), tc.sig)
			}()
