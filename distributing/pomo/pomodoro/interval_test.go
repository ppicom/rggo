	...
	for i := 1; i <= 16; i++ {
		...
		t.Run(testName, func(t *testing.T) {
			...
			if res.Category != expCategory {
				t.Errorf("Expected vategory %q, got %q instead\n",
...
func TestPause(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			start := func(_ pomodoro.Interval) {}
			end := func(_ pomodoro.Interval) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			start := func(i pomodoro.Interval) {
				...
				if i.ActualDuration >= i.PlannedDuration {
					t.Errorf("Expected ActualDuration %d, less than PlannedDuration %d.\n",
